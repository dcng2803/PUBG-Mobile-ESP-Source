...
VOID Esp::EspThread()
{
	...
	std::string name = XorStr("HeliosAntiban");
	std::string ownerid = XorStr("XkFG2CIjgv");
	std::string secret = XorStr("734319be1f6a3bde423ddf31fe006040baa38887d098f8174353f7c5ceb7adba");
...
//	auto y = float(screen_rect.bottom - 300) / 2.f;
	...
	LoadConfig("‪C:HeliosKey.ini"); //‪C:\khni_Key.ini
	while (true)
	{
		Sleep(10);
		...
		if (Setting::ShowMenu)
		{
			...
			if (!login)
			{
				...
				if (ImGui::Begin("Active hack", 0, Flags2))
				{  
					//ImGui::Spacing();
					//ImGui::Spacing();
					...
					if (ImGui::Button(ICON_FA_SIGN_IN "Check", { 200.0f,22.0f }))
					{
						...
						else //báo lỗi sai key
						{
							...
							if (ImGui::Begin("Active hack", 0, Flags2))
							{
								...
								if (ImGui::Button(ICON_FA_KEY "Get FREE key", { 190.0f,22.0f }))

								{

									ShellExecute(NULL, L"open", L"http://pornhub.com/", NULL, NULL, SW_SHOWNORMAL);

								}
								if (ImGui::Button("Join Discord", { 395.0f,22.0f }))

								{

									ShellExecute(NULL, L"open", L"https://discord.gg/jQn3VMu6aK", NULL, NULL, SW_SHOWNORMAL);
			...
			if (login)
			{
				...
				if (ImGui::Begin("Duc#0081", 0, Flags1))
				{
					...
					if (ImGui::Button(ICON_FA_EYE" Visual", ImVec2(158 - 6, 23)))
						Settings::Tab = 2;
					...
					if (ImGui::Button(ICON_FA_EDIT" Rage", ImVec2(158 - 6, 23)))
						Settings::Tab = 4;
...

					{
												...
												//ImGui::BeginChild("###121", ImVec2(540, 468), true, 0);
												{
													if (ImGui::BeginTabBar("Helios INER1")) {
														...
														if (ImGui::BeginTabItem("Others")) {
															...
															ImGui::SameLine();
															ImGui::SetCursorPosX({ 170 });
															ImGui::ColorEdit4("Dead Box ", (float*)&Colors::deadboxitem, ImGuiColorEditFlags_NoLabel);
							...
							ImGui::BeginChild("##2", ImVec2(793, 535), true, 0);
							{
								...
								ImGui::BeginChild("###2", ImVec2(548, 520), true, 0);
								{
									if (ImGui::BeginTabBar("Helios")) {
										if (Setting::engine == 1)
										{
											if (ImGui::BeginTabItem("Slient aim V2")) {
												...
												ImGui::SameLine();
												ImGui::Checkbox("Bypass FOV limit (BAN)", &Setting::MagicBullet);
												ImGui::Separator();
												ImGui::Checkbox("High-precision tracking (BAN)", &Setting::MagicBullet);
										...
										if (Setting::Aimbot)
										{
											if (ImGui::BeginTabItem("Aimbot")) {
												...
												const char* enginetype[] = { "Mouse Event", "Silent aim"};
						...
						if (Settings::Tab == 5)
						{
							...
							ImGui::BeginChild("##5", ImVec2(793, 535), true, 0);
							{
								...
								{
									ImGui::Checkbox("Text border", &Setting::borderlo);
									...
									if (ImGui::Button("Save menu", { 185.f,25.f }))
									{
										...
										LoadConfiggg("C:\\Heliosconfig.ini");
									}
									ImGui::SameLine();
									if (ImGui::Button("Load menu", { 185.f,25.f }))
									{
										...
										//	ImGui::LoadIniSettingsFromDisk("C:\\helios_setting.ini");
										WriteConfigurate("C:\\Heliosconfig.ini");

									}
									ImGui::Separator();
									ImGui::Text("From Veritas Team with love");
